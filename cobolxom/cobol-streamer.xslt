...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="t:format-line" as="xs:string*">
    ...
    <xsl:variable name="type" as="xs:QName?" select="
      $control-tokens[. = ($t:noformat, $t:code, $t:comment)][1]"/>
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:call-template name="t:format-long-code-line">
                  ...
                  <xsl:with-param name="continuation" select="false()"/>
                </xsl:call-template>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:otherwise>
    ...
    <xsl:if test="exists($breaker)">
      ...
      <xsl:if test="exists($next-tokens)">
        <xsl:call-template name="t:format-long-code-line">
          ...
          <xsl:with-param name="leader" select="$leader and not($line)"/>
          <xsl:with-param name="continuation" select="$next-continuation"/>
        </xsl:call-template>
      </xsl:if>
    </xsl:if>
  </xsl:template>
